...
y_train = df.iloc[0:150, 9].values
y_train = np.where(y_train == 'positive', -1, 1)

X_train = df.iloc[0:150, :9].values
X_train2 = []
for x in X_train:
    x2 = []
    for xi in x:
...
ada1 = a.AdalineGD(n_iter=15, eta=0.01).fit(X_train2, y_train)
ax[0].plot(range(1, len(ada1.cost_) + 1), np.log10(ada1.cost_), marker='o')
ax[0].set_xlabel('Epochs')
ax[0].set_ylabel('log(Sum-squared-error)')
ax[0].set_title('Adaline - Learning rate 0.01')

ada2 = a.AdalineGD(n_iter=15, eta=0.001).fit(X_train2, y_train)
ax[1].plot(range(1, len(ada2.cost_) + 1), np.log10(ada2.cost_), marker='o')
ax[1].set_xlabel('Epochs')
ax[1].set_ylabel('log(Sum-squared-error)')
ax[1].set_title('Adaline - Learning rate 0.001')

ada3 = a.AdalineGD(n_iter=15, eta=0.0001).fit(X_train2, y_train)
...
adaSGD = aSGD.AdalineSGD(n_iter=15, eta=0.0001).fit(X_train2, y_train)
